...
#+BEGIN_SRC emacs-lisp
  ...
  (use-package org
    ...
    (setq org-todo-keywords
          '((sequence "TODO" "IN-PROGRESS" "WAITING" "|" "DONE" "DEFERRED")))
    :bind
    (("C-c a" . 'org-agenda)
...
** swiper
Kind of an "interactive =M-x occur=" that integrates with [[*ivy][ivy]]
#+begin_src elisp
  (use-package swiper
  :ensure t
  :bind (("C-s" . swiper)
...
** org-download
See the [[https://github.com/abo-abo/org-download][project on github]].
The ~dired-mode~ hook enables drag-and-drop to dired.
The ~org-download-image-dir~ causes all downloads to be put in the /figures/ subdirectory.
#+begin_src elisp
  (use-package org-download
    ...
    :config (setq-default org-download-image-dir "./figures/"))
#+end_src
...
#+begin_src elisp
  (let ((reader-files
         (pcase system-type
           ('darwin '("*.el" "render-core.dylib"))
           ('gnu/linux '(:defaults "render-core.so"))
           (t nil))))
    (use-package reader
...
** projectile
Project specific compile commands can be set by specifying ~projectile-project-compilation-cmd~ in the project's /.dir-locals.el/.
#+begin_src emacs-lisp
...
#+begin_src elisp
  (use-package cider
    :ensure t
    :config
    (setq cider-repl-prompt-function (lambda (ns) (format "🫑 %s:> " ns))))
#+end_src
** ibuffer
#+begin_src emacs-lisp
...
** restclient
The following two packages allow to do something like
#+begin_example
#+begin_src restclient :results pure replace :exports none :jq .
